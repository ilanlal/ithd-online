...
class CachingUtils {
    private $config_path = array('caching_path' => './cache/',   'priority_required' => 0);
        ...
        if (is_array($this->config_path))
            ...
            if (file_exists($this->config_path)) {
                ...
                if (!isset($config['cache'])) {
                    echo "File $this->config_path can be reached but there is no variable called " . '$config["cache"].';
                    exit(-1);
                } else if (!is_array($config['cache'])) {
                    echo "File $this->config_path can be reached but the variable ";
                    exit(-1);
                } else {
                    $config = $config['cache'];
                }
                ...
                echo "$this->config_path doesn't point to a valid PHP configuration file.";
                exit(-1);
            }
        }
     ...
     */
    function set($name, $content, $duration = 0) {
    ...
    /**
     *   Return the content previously cached with set().
     *   @param string $name Has to be the same as used when calling set()
    ...
    function cache_buffer($name, $duration = 0) {
        ...
        if (!$this->has_buffer_started) {
            ...
            $this->set($name, $this->buffer_content, $duration);
        }
    }
